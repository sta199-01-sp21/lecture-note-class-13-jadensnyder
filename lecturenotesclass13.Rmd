...
library(infer)
library(usethis)
use_git_config(user.name= "jadensnyder", user.email="jaden.snyder@duke.edu")
```
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-jadensnyder/manhattan.csv")
```
...
- Population: 1 bedroom apartments in Manhatten
...
single code chunk with a single pipeline. Provide an interpretation of your 
result.
...
What does this mean?
We are 95% confident the true proportion of mice who died while not on treatment
is in this interval between .690 and .797.
...
We can be 90% confident that the true proportion of mice who lived while on the 
treatment regardless of the dose is between .211 and .303 or 21.1% and 30.3%.
